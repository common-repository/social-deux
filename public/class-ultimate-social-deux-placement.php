...
class UltimateSocialDeuxPlacement {
	...
	private function __construct() {
		...
		add_filter( 'the_content', 'wpautop', 4 );

		add_action( 'init', array( $this, 'load_textdomain' ) );

		add_filter( 'the_content', array( $this, 'buttons_pages_top'), 5 );

		add_filter( 'the_content', array( $this, 'buttons_pages_bottom'), 5 );

		add_filter( 'the_content', array( $this, 'buttons_posts_top'), 5 );

		add_filter( 'the_content', array( $this, 'buttons_posts_bottom'), 5 );
		...
		if (class_exists('Woocommerce')) {
			...
			add_action( 'woocommerce_after_single_product_summary', array( $this, 'woocommerce_bottom' ), 5 );
		}
		...
		if (class_exists('Easy_Digital_Downloads')) {
			add_filter( 'the_content', array( $this, 'buttons_edd_bottom'), 5 );
			add_filter( 'the_content', array( $this, 'buttons_edd_top'), 5 );
		}

		add_filter( 'the_content', array( $this, 'buttons_cpt_bottom'), 5 );

		add_filter( 'the_content', array( $this, 'buttons_cpt_top'), 5 );

		add_filter( 'the_content', array($GLOBALS['wp_embed'], 'autoembed'), -999);
	...
	public function remove_buttons_excerpts($text){
		...
		remove_filter( 'the_content', array( $this, 'buttons_posts_bottom'), 10 );
		remove_filter( 'the_content', array( $this, 'buttons_posts_top'), 10 );
		remove_filter( 'the_content', array( $this, 'buttons_edd_bottom'), 10 );
		remove_filter( 'the_content', array( $this, 'buttons_edd_top'), 10 );
	...
	public function remove_post_text_excerpts($text){
		...
		$sharetext_top = ( self::opt('us_posts_top_share_text', 'us_placement') ) ? self::opt('us_posts_top_share_text', 'us_placement') : '';
		$sharetext_bottom = ( self::opt('us_posts_bottom_share_text', 'us_placement') ) ? self::opt('us_posts_bottom_share_text', 'us_placement') : '';
